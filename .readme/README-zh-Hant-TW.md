...
- 按主程式提供的順序播放最多 128 個明確選取的音訊檔案, 支援上一首 / 下一首及可跳轉或移除曲目的播放佇列面板.
- 提供由播放服務持有的預設 / 自訂睡眠計時器, 播完本曲後停止, 最後 5 秒淡出及適合聽力練習的 A-B 區間循環.
- 只記住最近開啟的一個檔案及其位置, 開啟其他檔案時立即取代, 並向系統同步目前中繼資料和控制.
- 提供可選取最多 128 個文件的啟動頁面, 並接收唯讀 Android ACTION_VIEW `content` URI, 包括舊式 WMA MIME 別名.
- 解碼失敗時可使用其他相容應用程式開啟檔案, 並排除本外掛程式以防止自循環.
- 從檔案瀏覽器開啟單曲時, 透過請求級 Host Session 自動尋找可讀直接同層音訊並按自然順序建立有界佇列; 明確多選仍保持主程式選取順序.
- 由單一色彩來源產生清晰配色並跟隨 AutoJs6, 提供 19 個 Material 500 色及自訂 RGB; 設定也涵蓋語言、夜間、接續播放、更新、記錄及應用程式資訊.
...
Explorer 目錄宣告 `audio/*`, 並透過以下副檔名相容舊式或不完整 MIME 表:
...
安裝外掛程式並在 AutoJs6 外掛中心啟用後, 檔案管理器會為單一檔案顯示主要動作播放音訊, 並在多選工具列顯示播放所選音訊. 單一檔案入口從所選曲目開始, 可按自然順序尋找可讀直接同層音訊; 明確多選保持主程式提供的選取順序.
...
版本 1.3.0 提供 v12 唯讀動作、有限同層存取、獨立文件選擇器及 Android 音訊 / WMA 入口. 沒有可選 Host Session 時僅播放所選檔案.

同資料夾尋找需要 AutoJs6 6.8.0 build 5276 或更新版本及 Explorer Action v12; 後續外掛程式能力不會提高此要求.
...
外掛程式不要求儲存權限且不會寫入來源檔案. 網際網路只用於手動或每日 GitHub 發行版檢查. 簽章入口嚴格驗證 v12、TARGETS、ClipData、中繼資料和唯讀授權; Host Session 綁定 UID 且不遞迴, 公開入口保持唯讀.
...
- 單一檔案動作從剛好 1 個所選檔案開始, 可建立可讀直接同層音訊的有界佇列; 多選動作接受 1 至 128 個不重複檔案並保持順序.
- 宣告大小超過 8 TiB 的檔案瀏覽器要求會被拒絕.
- 動作依已辨識音訊 MIME 或明確副檔名清單比對, 並正規化及驗證每個目標.
- 同層尋找僅可透過主程式管理的請求級工作階段且不遞迴; 外掛程式絕不猜測同層 URI, 也不接收檔案系統路徑.
...
* `新增` 語言、夜間模式和色彩來源預設透過 AutoJs6 官方唯讀設定契約跟隨主程式; 主程式不可用時仍顯示但停用相應選項, 並回復至應用程式預設值
* `新增` 新增手動與每日自動檢查更新、已忽略版本管理及本地化內建發行記錄
* `修復` 修正跟隨 AutoJs6 一直顯示 Host color unavailable 的問題; 外掛程式現已公開主程式設定提供者要求的受保護外掛程式資訊服務入口
* `修復` Explorer 動作除 19 個已知副檔名外也宣告音訊 MIME 支援並新增 WMA, 讓主程式辨識的所有音訊項目均可直接使用外掛程式
* `優化` 接續播放現在只記住最近開啟的一個檔案, 開啟其他檔案時立即捨棄舊記錄, 播放完畢不會保留位置
* `優化` 固定保留三行中繼資料區域, 並結合實際選取音軌補充取樣率與位元率, 依 44.1 kHz · MP3 · 128 kbps 的順序顯示
* `優化` A-B 改為依序設定 A 點、B 點和清除的三次點選循環; 底部控制區增加間距, 圖示統一大小並嚴格垂直置中

# v1.2.2

###### 2026/08/27

* `修復` 修復覆蓋安裝外掛後, 執行中的 AutoJs6 檔案管理器仍傳送已快取的協定 v4 動作, 導致所有音訊立即顯示「音訊請求無效」的問題; 閘道現在相容唯讀 v4–v12 請求, 對外仍宣告 v12
* `修復` 修復部分 Android / OEM MIME 表將已宣告支援的音訊副檔名標記為萬用字元或 application 類型時遭誤拒的問題; 現由副檔名允許清單提供穩定的標準音訊 MIME 類型
* `優化` Explorer 請求拒絕日誌新增不含檔名, 顯示路徑或 URI 的隱私安全原因碼, 後續可直接定位契約差異

# v1.2.1

###### 2026/08/27

* `新增` 單檔「播放音訊」動作現在可透過 Explorer Action v12 探索同資料夾最多 128 個可讀音訊檔案, 並建立從選取曲目開始的自然排序佇列
* `修復` 修復 API 24 因 Android 從閘道 Activity 資訊清單自動附加非權限型 FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS 旗標而誤拒合法 Explorer 請求的問題
* `修復` 修復自動切換至同層曲目時重建 MediaSession 返回 Intent 所造成的崩潰; 原始授權目標現在獨立於目前播放項目持續作為 Host Session 錨點
* `優化` 選取曲目保留原始 content URI, 同層曲目僅透過請求級 Host Session 檔案描述元串流; 不猜測同層 URI, 不增加遞迴, 寫入, 儲存空間或永久存取
* `優化` 音訊副檔名篩選會將與 .m4a 同名的 .mp4 影片排除於佇列之外, 現有明確多選佇列行為維持不變
* `優化` Host Session 擁有權會轉交背景播放服務, 並在取代佇列, 啟動失敗, 播放完成或服務銷毀時關閉
* `相依性` 將內建 Explorer Action API 從通訊協定 v4 升級至向下相容的 v12 同層讀取擴充, 最低主程式建置版本仍為 5276
