...
class PluginContractInstrumentationTest {
    ...
    fun catalogAdvertisesSingleAndSelectionQueueActions() {
        val catalog = threeTerraPlayerActionCatalog()
        val actions = requireNotNull(
            catalog.getParcelableArrayList<android.os.Bundle>(ExplorerActionCatalogKeys.ACTIONS),
        )

        assertEquals(ThreeTerraPlayerPlugin.PROTOCOL_VERSION, catalog.getInt(ExplorerActionCatalogKeys.PROTOCOL_VERSION))
        assertEquals(2, actions.size)
        assertEquals(ThreeTerraPlayerPlugin.ACTION_ID, actions[0].getString(ExplorerActionCatalogKeys.ID))
        assertEquals(
            ExplorerActionValues.CARDINALITY_SINGLE,
            actions[0].getInt(ExplorerActionCatalogKeys.CARDINALITY),
        )
        assertEquals(ThreeTerraPlayerPlugin.ACTION_SELECTION_ID, actions[1].getString(ExplorerActionCatalogKeys.ID))
        assertEquals(
