...
<div align="center">
  ...
  <p>Lecture audio en lecture seule pour l'explorateur AutoJs6 avec commandes Media3 en arrière-plan</p>
...
Le plugin AutoJs6 Audio Player ajoute un contrôleur audio intégré et un service privé de lecture en arrière-plan pour les fichiers ouverts depuis l'explorateur AutoJs6. Il peut également recevoir des requêtes Android ACTION_VIEW en lecture seule pour des content URI avec un type MIME audio.
...
Lorsque le plugin est installé, l'explorateur AutoJs6 affiche Lire l'audio comme action principale pour les extensions indiquées. La sélection ouvre le contrôleur du plugin et démarre le service privé avec un accès temporaire en lecture seule au fichier choisi.

Lorsque le plugin est absent, cette action ne figure pas dans la liste. AutoJs6 conserve son flux ACTION_VIEW externe en lecture seule pour les fichiers audio, de sorte qu'une autre application audio installée peut traiter le fichier. Sans application externe compatible, l'hôte ne dispose pas d'une interface de lecture de remplacement.
...
La version 1 fournit une action principale en lecture seule pour un seul fichier dans l'explorateur AutoJs6 principal. Le catalogue utilise uniquement les extensions, tandis que l'entrée Android indépendante continue d'accepter `audio/*`.

Le plugin ne contient aucune bibliothèque native. Il déclare `supportedAbis = emptyArray()` et est publié sous la forme d'un APK unique indépendant de l'ABI. La version 5269 ou ultérieure de l'hôte AutoJs6 est requise.
