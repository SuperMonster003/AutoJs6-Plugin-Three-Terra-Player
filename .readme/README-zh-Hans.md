...
版本 1.4.0 提供协议 v12 单文件与有序多选只读动作、请求级直接同级读取能力、独立文档选择器以及公开只读 Android 音频 / WMA 入口. 不提供可选 Host Session 的宿主继续仅播放所选文件.
...
* `新增` 系统媒体通知采用专用上一曲、下一曲、随机播放开关和退出动作, 并使用应用的单色透明图标; 随机播放的明暗状态会与播放器即时同步
* `修复` 修复插件信息与 Explorer Action 共用同一 Service 时因 Binder 描述符错配而被 AutoJs6 自动标记为错误并禁用的问题; 两套协议现使用独立端点
* `修复` 播放队列清空后会立即清除过期曲目信息和后台会话, 禁用播放、跳转、队列、速度、定时器及 A-B 控件, 并以明确空状态替代无效点击
* `修复` 播放主按钮的阴影不再被底部区域裁切, 禁用时会使用明确的低强调配色; 设置及更新对话框的单选、多选、进度与按钮控件现统一跟随动态主题色
* `优化` 应用与插件统一更名为不可翻译的 3-Terra Player, 源码命名空间和符号同步采用 three / Three 形式, 同时保留已发布应用 ID 以兼容覆盖升级及既有设置
* `优化` 播放器右上角改为仅含“设置”的更多菜单, 移除与设置页重复的独立调色盘入口, 并保持播放控制区既有间距
