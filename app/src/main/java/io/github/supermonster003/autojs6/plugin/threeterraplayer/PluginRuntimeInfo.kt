...
internal object ThreeTerraPlayerPlugin {
    ...
    const val ACTIVITY_CLASS_NAME =
        "io.github.supermonster003.autojs6.plugin.threeterraplayer.ExplorerActionActivity"
...
internal fun Context.threeTerraPlayerPluginInfo(): PluginInfo {
    ...
    return PluginInfo().apply {
        ...
        id = ThreeTerraPlayerPlugin.ID
        engine = ExplorerActionPluginIds.ENGINE
        variant = ThreeTerraPlayerPlugin.VARIANT
        supportedAbis = emptyArray()
        capabilities = Bundle().apply {
            putLong(PluginCapabilityKeys.REQUIRES_HOST_VERSION, ThreeTerraPlayerPlugin.REQUIRED_HOST_VERSION)
            putInt(ExplorerActionCapabilityKeys.PROTOCOL_VERSION, ThreeTerraPlayerPlugin.PROTOCOL_VERSION)
        }
    }
}

internal fun threeTerraPlayerActionCatalog(): Bundle {
    return Bundle().apply {
        putInt(ExplorerActionCatalogKeys.PROTOCOL_VERSION, ThreeTerraPlayerPlugin.PROTOCOL_VERSION)
        putParcelableArrayList(
            ExplorerActionCatalogKeys.ACTIONS,
            arrayListOf(
                audioPlayerAction(
                    id = ThreeTerraPlayerPlugin.ACTION_ID,
                    labelResourceName = ThreeTerraPlayerPlugin.LABEL_RESOURCE_NAME,
                    labelFallback = ThreeTerraPlayerPlugin.LABEL_FALLBACK,
                    cardinality = ExplorerActionValues.CARDINALITY_SINGLE,
                    placement = ThreeTerraPlayerPlugin.PLACEMENT_PRIMARY,
                    readSiblings = true,
                ),
                audioPlayerAction(
                    id = ThreeTerraPlayerPlugin.ACTION_SELECTION_ID,
                    labelResourceName = ThreeTerraPlayerPlugin.SELECTION_LABEL_RESOURCE_NAME,
                    labelFallback = ThreeTerraPlayerPlugin.SELECTION_LABEL_FALLBACK,
...
private fun audioPlayerAction(
    ...
    putString(ExplorerActionCatalogKeys.ACTIVITY_CLASS_NAME, ThreeTerraPlayerPlugin.ACTIVITY_CLASS_NAME)
    putInt(ExplorerActionCatalogKeys.PRIORITY, ThreeTerraPlayerPlugin.ACTION_PRIORITY)
