...
<div align="center">
  ...
  <p>تشغيل صوت للقراءة فقط في مستكشف AutoJs6 مع عناصر تحكم Media3 في الخلفية</p>
...
يضيف ملحق AutoJs6 Audio Player وحدة تحكم صوت داخل التطبيق وخدمة خاصة للتشغيل في الخلفية للملفات المفتوحة من مستكشف AutoJs6. ويمكنه ايضا استقبال طلبات Android ACTION_VIEW للقراءة فقط لعناوين content URI ذات نوع MIME صوتي.
...
عند تثبيت الملحق يعرض مستكشف AutoJs6 تشغيل الصوت كاجراء رئيسي للامتدادات المدرجة. يؤدي اختياره الى فتح وحدة تحكم الملحق وبدء خدمة التشغيل الخاصة مع وصول مؤقت للقراءة فقط الى الملف المحدد.

عند غياب الملحق لا يظهر هذا الاجراء. يحتفظ AutoJs6 بمسار ACTION_VIEW الخارجي الحالي للقراءة فقط لملفات الصوت, لذلك يمكن لتطبيق صوت اخر مثبت معالجة الملف. اذا لم يتوفر تطبيق خارجي متوافق فلن يحصل المضيف على واجهة تشغيل بديلة.
...
يوفر الاصدار 1 اجراء رئيسيا للقراءة فقط لملف واحد في مستكشف AutoJs6 الرئيسي. يستخدم الكتالوج الامتدادات فقط بينما تواصل نقطة دخول Android المستقلة قبول `audio/*`.

لا يحتوي الملحق على مكتبة اصلية. يعلن `supportedAbis = emptyArray()` وينشر كملف APK واحد مستقل عن ABI. يتطلب بناء المضيف AutoJs6 رقم 5269 او احدث.
...
لا يطلب الملحق اذن التخزين او الشبكة. تحمي نقطة دخول المستكشف صلاحية توقيع AutoJs6 وتتحقق بشكل صارم من البروتوكول v2 وعناوين content URI الهدف والاصل وClipData وسطح المصدر واسم العرض ونوع MIME والحجم المعلن وعلامات القراءة فقط. لا يتم تمرير سوى URI الهدف واذن القراءة الى مكونات التشغيل الخاصة. تقبل نقطة دخول Android العامة طلبات content audio للقراءة فقط وترفض اذونات الكتابة والاذونات الدائمة واذونات البادئة ولا تمرر ابدا extras عشوائية من المتصل.
