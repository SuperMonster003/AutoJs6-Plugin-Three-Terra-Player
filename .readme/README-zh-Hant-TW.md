...
- 由一個色彩來源產生兼顧可讀性的淺色與深色配色, 預設跟隨 AutoJs6, 並提供 19 個本地化 Material 500 預設色及即時預覽的自訂 RGB 顏色.
...
安裝外掛程式後, 檔案管理器會為單一檔案顯示主要動作播放音訊, 並在多選工具列顯示播放所選音訊. 單一檔案入口從所選曲目開始, 可按自然順序尋找可讀直接同層音訊; 明確多選保持主程式提供的選取順序.
...
版本 1.2.1 提供通訊協定 v12 單一檔案唯讀主要動作, 可接收請求級直接同層讀取能力, 並保留有序多選唯讀動作. 獨立 Android 入口仍為單一檔案並接受 `audio/*`. 不提供可選 Host Session 的主程式繼續僅播放所選檔案.
...
外掛程式不要求儲存空間或網際網路存取權限. 受簽章權限保護的入口嚴格驗證通訊協定 v12, 有序 TARGETS 與 ClipData, 識別碼, 父子關係, 中繼資料和唯讀旗標. 可選 Host Session 由主程式綁定至外掛程式 UID, 僅可列出所選檔案的直接上層資料夾並開啟所選檔案或可讀直接同層檔案; 播放元件只接收不透明合成路由, 不接收檔案系統路徑. 公開 Android 入口保持唯讀單一檔案.
...
* `優化` 檔案瀏覽器要求驗證擴展至有序 TARGETS 與 ClipData, 唯一請求和目標識別碼, 主程式工作階段描述及每個選取檔案, 且不擴大唯讀授權
* `優化` 明確記錄主程式 FileProvider 上層目錄 URI 無法列舉子項; 繼續停用同層檔案自動尋找和 URI 猜測, 並以明確多選作為安全播放佇列路徑
* `相依性` 內建檔案瀏覽器動作 API 從通訊協定 v2 升級至 v4, 最低主程式建置版本提升至 5276
