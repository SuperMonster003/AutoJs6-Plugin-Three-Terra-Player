...
internal object ExplorerActionIntentPolicy {
    ...
    private fun resolveUnchecked(intent: Intent?): ExplorerAudioRequest {
        ...
        val multipleAction = when (actionId) {
            ThreeTerraPlayerPlugin.ACTION_ID -> false
            ThreeTerraPlayerPlugin.ACTION_SELECTION_ID -> true
        ...
        if (
            protocolVersion !in
            ThreeTerraPlayerPlugin.MIN_COMPATIBLE_REQUEST_PROTOCOL_VERSION..ThreeTerraPlayerPlugin.PROTOCOL_VERSION
        ) {
        ...
        if (hostVersion < ThreeTerraPlayerPlugin.REQUIRED_HOST_VERSION) reject(ExplorerRequestRejection.HOST_VERSION)
