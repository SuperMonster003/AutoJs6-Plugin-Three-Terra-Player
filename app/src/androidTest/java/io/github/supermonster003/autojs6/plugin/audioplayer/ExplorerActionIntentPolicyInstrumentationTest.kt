...
class ExplorerActionIntentPolicyInstrumentationTest {
    ...
    fun acceptsCompatibleV4RequestFromStaleHostActionCache() {
        val intent = validIntent(
            ...
            multiple = false,
            protocolVersion = AudioPlayerPlugin.MIN_COMPATIBLE_REQUEST_PROTOCOL_VERSION,
        )
    ...
    fun rejectsProtocolOutsideCompatibilityWindow() {
        val belowFloor = validIntent(
            targets = listOf(target("a", "first.mp3")),
            multiple = false,
            protocolVersion = AudioPlayerPlugin.MIN_COMPATIBLE_REQUEST_PROTOCOL_VERSION - 1,
        )
        val aboveCurrent = validIntent(
            targets = listOf(target("a", "first.mp3")),
            multiple = false,
            protocolVersion = AudioPlayerPlugin.PROTOCOL_VERSION + 1,
        )
    ...
    fun acceptsEveryAdvertisedExtensionWithPlatformMimeMap() {
        val rejected = AudioPlayerPlugin.EXTENSIONS.mapNotNull { extension ->
    ...
    private fun validIntent(
        targets: List<Bundle>,
        multiple: Boolean,
        protocolVersion: Int = AudioPlayerPlugin.PROTOCOL_VERSION,
    ): Intent {
        ...
        return Intent(ExplorerActionPluginActions.EXECUTE).apply {
            ...
            putExtra(
                ...
                if (multiple) AudioPlayerPlugin.ACTION_SELECTION_ID else AudioPlayerPlugin.ACTION_ID,
            )
            ...
            putExtra(ExplorerActionIntentExtras.HOST_VERSION_CODE, AudioPlayerPlugin.REQUIRED_HOST_VERSION)
            putExtra(
