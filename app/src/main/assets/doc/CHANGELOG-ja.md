...
* `修正` プラグインを上書きインストールした後, ファイルマネージャーのすべての音声で音声リクエストが無効と表示される問題を修正: 入口は実行中のホストがキャッシュした旧版の読み取り専用リクエストも受け付けるように
* `修正` 一部の端末の MIME テーブル異常により `ogg`, `opus` などの対応済み拡張子が誤って拒否される問題を修正: 拡張子の許可リストが安定した正規音声タイプを提供
* `改善` リクエストが拒否された際に, プライバシーに安全な理由コードをログに出力 (ファイル名, パス, URI を含まない) し, フィードバックと問題の特定を容易に

# v1.2.1

###### 2026/08/27

* `機能` ファイルマネージャーから 1 曲を開くと, 同じフォルダーの他の音声を自動検出し, ファイル名の自然順で連続再生: 最大 128 曲, 選択した曲から開始
* `修正` Android 7.0 端末でシステムが自動付加するウィンドウフラグにより, 正当な再生リクエストが誤って拒否される問題を修正
* `修正` 同じフォルダーの曲へ自動的に切り替わる際にクラッシュすることがある問題を修正
* `改善` 同じフォルダーの検出は常に読み取り専用かつ境界付き: サブフォルダーへ再帰せず, ファイルの場所を推測せず, ストレージ権限も要求せず, 音声と同名の動画ファイルはキューに入らない
* `改善` 同じフォルダーの読み取りセッションはバックグラウンド再生サービスが保持し, キューの置き換え, 再生完了, サービス破棄の際に自動的にクローズ
* `依存関係` 同梱のファイルマネージャーアクションインターフェースを後方互換の v12 へ更新し, 最低ホストバージョンは build 5276 を維持

# v1.2.0

###### 2026/08/27

* `機能` 複数選択再生を追加: ファイルマネージャーで最大 128 個の音声を選択してワンタップでキューに追加し, 選択した順序を厳密に維持
* `機能` 前の曲 / 次の曲の操作, 曲へのジャンプと削除ができるキューパネル, 順番 / シャッフル / 1 曲リピートのモードを追加
* `機能` スリープタイマーを追加: 15 / 30 / 60 分のプリセットまたは任意の時間に対応し, 現在の曲の終了後に停止と最後の 5 秒のフェードアウトも可能
* `機能` 選んだ区間を繰り返し聴ける A-B リピートを追加
* `改善` システムメディアコントロールに前の曲, 次の曲, 10 秒の早戻し / 早送りコマンドを追加し, 通知のメタデータはキューの切り替えに合わせてリアルタイムに更新
* `改善` ファイルマネージャーのリクエスト検証を順序付き複数選択の対象へ拡張し, 最小限の読み取り専用権限付与を維持
* `依存関係` ファイルマネージャーアクションインターフェースを v2 から v4 へ更新し, 最低ホストバージョンを build 5276 へ引き上げ
* `依存関係` AndroidX RecyclerView 1.4.0 を追加

# v1.1.0

###### 2026/08/27

* `機能` 新しい再生画面: アルバムアート, タイトル / アーティスト / アルバムのタグ, 技術情報, ドラッグできるシークバー
* `機能` 10 秒の早戻し / 早送り, 1 曲リピートの切り替え, 0.5 倍から 2 倍までの速度調整を追加
* `機能` 音声に埋め込まれたタグとカバーアートを自動で読み取り, システムメディア通知に同期
* `機能` 続きから再生を追加: 同じファイルを再び開くと前回の位置から再開し, 再生完了後は記録を自動消去
* `改善` 再生終了やデコード失敗の後も画面内でそのまま再生し直せるため, ファイルマネージャーに戻る必要なし
* `改善` 再生エラーの表示に具体的なエラーコードを付加し, 調査と報告を容易に
* `依存関係` AndroidX ConstraintLayout 2.2.1 を追加
* `依存関係` 使用されなくなった AndroidX Media3 UI 依存関係を削除

# v1.0.1

###### 2026/08/08

* `修正` プラグインセンターでプラグインを有効化する際に, 空のサービスバインディングが発生する問題を修正
* `改善` プラグインの名前, 説明, 使用説明をより簡潔で自然に

# v1.0.0

###### 2026/08/02

* `機能` 初の正式バージョン: AutoJs6 のファイルマネージャーに音声再生アクションを提供し, 18 種類の一般的な音声拡張子をカバー, タップするだけで再生
* `機能` Media3 ExoPlayer と MediaSessionService を基盤とする安定した再生: バックグラウンド再生, 音声フォーカス管理, 出力デバイス切断時の自動一時停止, システムメディアコントロールに対応
* `機能` 他のアプリからの読み取り専用の音声オープンリクエストの受信に対応; デコード失敗時は他の互換アプリへ引き渡し, 自己ループを防止
* `機能` 厳格なセキュリティベースライン: ストレージ権限もインターネット権限も要求せず, 最小限の読み取り専用権限付与だけを受け入れて転送
* `機能` プラグイン情報, 使用説明, README, 更新履歴が 10 言語に対応
* `依存関係` AndroidX Media3 ExoPlayer, Session, UI 1.10.1 を追加
